...
class PytoolsSetupCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        # setup
        option = {"conda":self.conda_setup, "venv":self.venv_setup, "python_executable":self.python_setup}
        # conda quickpane
        pass
    ...
    def python_setup(self):
        # get input path
        python = ""
        if not os.isfile(python):
            print("python not found", python)
        listdir = os.listdir(os.path.dirname(python))
        if ["activate.bat","activate.sh"] in listdir:
            pass
        pass
    ...
    def venv_setup(self):
        ...
        path = ""
        bin_name = "Scripts" if os.name == "nt" else "bin"
        ext_name = ".exe" if os.name == "nt" else ""
        python = os.path.join(path,bin_name,"python",ext_name)
        if not os.isfile(python):
            pass
        pass
